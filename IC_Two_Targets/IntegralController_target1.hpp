...
#ifndef INTEGRALCONTROLLER_TARGET1
#define INTEGRALCONTROLLER_TARGET1
#include "mechanism.hpp"
#include <limits>

//inherit controller class spec
class IntegralController_target1: public mechanism {
...
public:
    ...
    IntegralController_target1(double tau_m_, double tau_g_, double m_)
    {
        ...
        if (tau_g<=0) {mexErrMsgTxt("[IntegralController_target1] tau_g must be > 0. Perhaps you meant to set it to Inf?\n");}
    }
...
double IntegralController_target1::getState(int idx)
{
    if (idx == 1) {return m;}
...
int IntegralController_target1::getFullStateSize(){return 2; }


int IntegralController_target1::getFullState(double *cont_state, int idx)
{
...
void IntegralController_target1::connect(conductance * channel_)
{
...
void IntegralController_target1::connect(compartment* comp_)
{
    mexErrMsgTxt("[IntegralController_target1] This mechanism cannot connect to a compartment object");
}

void IntegralController_target1::connect(synapse* syn_)
{
...
void IntegralController_target1::integrate(void)
{
    ...
    {
        case 0:
            mexErrMsgTxt("[IntegralController_target1] misconfigured controller. Make sure this object is contained by a conductance or synapse object");
            break;
        ...
        default:
            mexErrMsgTxt("[IntegralController_target1] misconfigured controller");
            break;
...
void IntegralController_target1::checkSolvers(int k) {
    if (k == 0){
        return;
    } else {
        mexErrMsgTxt("[IntegralController_target1] unsupported solver order\n");
    }
}
