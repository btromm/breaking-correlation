...
#ifndef INTEGRALCONTROLLER_ICA
#define INTEGRALCONTROLLER_ICA
#include "mechanism.hpp"
#include <limits>

//inherit controller class spec
class IntegralController_ICa: public mechanism {
...
public:
    ...
    IntegralController_ICa(double tau_m_, double tau_g_, double m_)
    {
        ...
        // if (tau_m<=0) {mexErrMsgTxt("[IntegralController_ICa] tau_m must be > 0. Perhaps you meant to set it to Inf?\n");}
        if (tau_g<=0) {mexErrMsgTxt("[IntegralController_ICa] tau_g must be > 0. Perhaps you meant to set it to Inf?\n");}
    }
...
double IntegralController_ICa::getState(int idx)
{
    if (idx == 1) {return m;}
...
int IntegralController_ICa::getFullStateSize(){return 2; }


int IntegralController_ICa::getFullState(double *cont_state, int idx)
{
...
void IntegralController_ICa::connect(conductance * channel_)
{
...
void IntegralController_ICa::connect(compartment* comp_)
{
    mexErrMsgTxt("[IntegralController_ICa] This mechanism cannot connect to a compartment object");
}

void IntegralController_ICa::connect(synapse* syn_)
{
...
void IntegralController_ICa::integrate(void)
{
    ...
    {
        case 0:
            mexErrMsgTxt("[IntegralController_ICa] misconfigured controller. Make sure this object is contained by a conductance or synapse object");
            break;
        ...
        default:
            mexErrMsgTxt("[IntegralController_ICa] misconfigured controller");
            break;
...
void IntegralController_ICa::checkSolvers(int k) {
    if (k == 0){
        return;
    } else {
        mexErrMsgTxt("[IntegralController_ICa] unsupported solver order\n");
    }
}
